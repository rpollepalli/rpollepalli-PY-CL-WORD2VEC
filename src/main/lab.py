...
# TODO: complete the following function to retrieve vector representation of an input word. 
# You should also make sure if it doesn't know the word, 
# it returns -1, it will not crash.(aka, catch the KeyError)
def retrieveVector(input_word):
    
    try:
        vec_input_word = w2v[input_word]
        return vec_input_word
    except KeyError as e:
        print(e)
        return -1
"""
...
# TODO: Complete the following function so that it calculates the similarity between two words 
# and the similarity index is greater than 0.4
def similarPairExercise():
    word1 = "cap"
    word2 = "hat"
    return w2v.similarity(word1, word2)

# TODO: Complete the following function so that w2v model retrieves top 'max_limit' 
# number of the words you provide 
def retrieveSimilarWordsExercise(max_limit: int):
    ...
    return w2v.most_similar(positive=words, topn=max_limit)
    #grab max_limit number of words similar to words appearing in words
...
def w2vDoesntMatchExercise(words: list[str]):
    lsw=w2v.doesnt_match(words)
    print(lsw)
    return lsw
    # return the least similar worsd in the words list 